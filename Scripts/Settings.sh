...
if [ -n "$AGH_CORE" ]; then
    # 精确提取可执行文件（兼容新老压缩包结构）
    if wget -qO- "$AGH_CORE" | tar -xOz --wildcards '*/AdGuardHome' > files/usr/bin/AdGuardHome/AdGuardHome 2>/dev/null; then
        chmod +x files/usr/bin/AdGuardHome/AdGuardHome
        green "✅ AdGuardHome 下载完成 (${ARCH})"
    else
        # 若通配符失败，尝试直接解压全部（旧版结构）
        wget -qO- "$AGH_CORE" | tar -xOz > files/usr/bin/AdGuardHome/AdGuardHome
        chmod +x files/usr/bin/AdGuardHome/AdGuardHome
        green "✅ AdGuardHome 下载完成 (兼容模式)"
    fi
else
    yellow "⚠️ 未找到 ${ARCH} 架构的 AdGuardHome，跳过"
fi
# ---- 完成 ----
green ""
green "========================================="
